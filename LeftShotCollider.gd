...
func _ready():
	print("LeftShotCollider connecting 'player_fired_focus_beam_left' signal with '_on_Player_fired_focus_beam' handler...")
	player = get_node("../../../Player1")
	parent = get_parent()
	var err = player.connect("player_fired_focus_beam_left", self,"_on_Player_fired_focus_beam")
	if err:
		print(err)
